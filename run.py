#!/usr/bin/env python3
# Script used by Kaggle competition winners to train model. See https://github.com/guestwalk/kaggle-2014-criteo
...
cmd = './gbdt -t 30 -s {nr_thread} tr.gbdt.dense tr.gbdt.sparse tr.gbdt.dense tr.gbdt.sparse tr.gbdt.out tr.gbdt.out2'.format(nr_thread=NR_THREAD) 
subprocess.call(cmd, shell=True)

cmd = 'rm -f tr.gbdt.dense tr.gbdt.sparse'
subprocess.call(cmd, shell=True)
...
cmd = 'rm -f tr.gbdt.out tr.gbdt.out2'
subprocess.call(cmd, shell=True) 

cmd = './ffm-train -k 4 -t 18 -s {nr_thread} tr.ffm model'.format(nr_thread=NR_THREAD) 
subprocess.call(cmd, shell=True)
